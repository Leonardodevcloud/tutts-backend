...
async function createTables() {
  try {
    ...
    // Tabela de configuração da promoção novos profissionais
    await pool.query(`
      CREATE TABLE IF NOT EXISTS promo_novos_config (
        id SERIAL PRIMARY KEY,
        codigo_minimo INTEGER DEFAULT 1200,
        valor_gratuidade INTEGER DEFAULT 500,
        ativo BOOLEAN DEFAULT TRUE,
        created_at TIMESTAMP DEFAULT NOW(),
        updated_at TIMESTAMP DEFAULT NOW()
      )
    `);
    console.log('✅ Tabela promo_novos_config verificada');

    // Tabela de respostas do questionário
    await pool.query(`
      CREATE TABLE IF NOT EXISTS promo_novos_respostas (
        id SERIAL PRIMARY KEY,
        user_cod VARCHAR(50) NOT NULL,
        user_name VARCHAR(255),
        acertos INTEGER,
    ...
    `);
    console.log('✅ Tabela promo_novos_respostas verificada');
...
// ========== PROMOÇÃO NOVOS PROFISSIONAIS ==========

// Obter configuração
app.get('/api/promo-novos/config', async (req, res) => {
  try {
    const result = await pool.query('SELECT * FROM promo_novos_config ORDER BY id DESC LIMIT 1');
    if (result.rows.length === 0) {
      // Retorna configuração padrão
      return res.json({ codigoMinimo: 1200, valorGratuidade: 500, ativo: true });
    }
    res.json({
      codigoMinimo: result.rows[0].codigo_minimo,
      valorGratuidade: result.rows[0].valor_gratuidade,
      ativo: result.rows[0].ativo
    });
  } catch (error) {
    console.error('❌ Erro ao obter config:', error);
    res.json({ codigoMinimo: 1200, valorGratuidade: 500, ativo: true });
  }
});

// Salvar configuração
app.post('/api/promo-novos/config', async (req, res) => {
  try {
    const { codigoMinimo, valorGratuidade, ativo } = req.body;
    
    // Verifica se já existe
    const existing = await pool.query('SELECT * FROM promo_novos_config LIMIT 1');
    
    if (existing.rows.length > 0) {
      await pool.query(
        'UPDATE promo_novos_config SET codigo_minimo = $1, valor_gratuidade = $2, ativo = $3, updated_at = NOW() WHERE id = $4',
        [codigoMinimo, valorGratuidade, ativo, existing.rows[0].id]
      );
    } else {
      await pool.query(
        'INSERT INTO promo_novos_config (codigo_minimo, valor_gratuidade, ativo) VALUES ($1, $2, $3)',
        [codigoMinimo, valorGratuidade, ativo]
      );
    }
...
app.get('/api/promo-novos/verificar/:userCod', async (req, res) => {
  try {
    const { userCod } = req.params;
    const result = await pool.query(
      'SELECT * FROM promo_novos_respostas WHERE user_cod = $1',
      [userCod]
    );
    res.json({ respondido: result.rows.length > 0 });
  } catch (error) {
    console.error('❌ Erro ao verificar:', error);
    res.json({ respondido: false });
  }
});

// Responder questionário
app.post('/api/promo-novos/responder', async (req, res) => {
  try {
    const { userCod, userName, acertos, passou, valorGratuidade } = req.body;
    ...
    if (existing.rows.length > 0) {
      return res.status(400).json({ error: 'Já respondeu o questionário' });
    }
    
    // Registra a resposta
    await pool.query(
      'INSERT INTO promo_novos_respostas (user_cod, user_name, acertos, passou, created_at) VALUES ($1, $2, $3, $4, NOW())',
      [userCod, userName, acertos, passou]
    );
    
    // Se passou, cria a gratuidade
    if (passou) {
      await pool.query(
        `INSERT INTO gratuities (user_cod, quantity, remaining, value, reason, status, created_at)
         VALUES ($1, 1, 1, $2, 'Questionário Novo Profissional', 'ativa', NOW())`,
        [userCod, valorGratuidade / 100]
      );
      console.log(`✅ Gratuidade criada para ${userName} (${userCod}): R$ ${(valorGratuidade / 100).toFixed(2)}`);
    }
    
    res.json({ success: true, passou, acertos });
  } catch (error) {
