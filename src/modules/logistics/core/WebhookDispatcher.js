 ...
 *  5. Localiza entrega em uber_entregas pelo external_delivery_id
 *  6. IDEMPOTÊNCIA: se status já processado, ignora
 *  7. Atualiza uber_entregas
 ...
 *
 * IMPORTANTE: este código escreve em uber_entregas (Opção A — escolha do
...
class WebhookDispatcher {
  ...
  async handle(providerCode, req, res) {
    ...
    // Evento sem ação (refund, sem delivery_id, etc) → ack e fim
    if (!evento || evento.eventType === 'other' || !evento.externalDeliveryId) {
      console.log(`📨 [WebhookDispatcher] ${providerCode}: evento sem ação (${evento?.eventType || 'null'})`);
      return adapter.acknowledgeWebhook(res);
    }
  ...
  async _processarEvento(providerCode, adapter, evento) {
    ...
    if (entrega.status_uber === novoStatusNative) {
      console.log(`⊘ [WebhookDispatcher] OS ${codigoOS}: status ${novoStatusNative} duplicado, ignorado`);
      return;
    }

    console.log(`📡 [WebhookDispatcher] OS ${codigoOS}: ${entrega.status_uber} → ${novoStatusNative} (${novoStatusCanonico})`);
    ...
    if (evento.trackingUrl && !entrega.tracking_url) {
      await this.pool.query(
        ...
        [novoStatusNative, evento.trackingUrl, entrega.id]
      );
    } else {
      await this.pool.query(
        'UPDATE uber_entregas SET status_uber = $1, updated_at = NOW() WHERE id = $2',
        [novoStatusNative, entrega.id]
      );
    }
    ...
    try {
      switch (acao.type) {
        ...
        case 'alterar_status':
          ...
          await this.pool.query(
            'UPDATE uber_entregas SET cancelado_por = $1, cancelado_motivo = $2, updated_at = NOW() WHERE id = $3',
            ['uber', `Status: ${evento.statusNative}`, entrega.id]
          );
          break;
  ...
  async _processarCourierUpdate(providerCode, entrega, evento) {
    ...
    // Vincula motorista na Mapp se é a primeira vez (entrega ainda sem entregador_nome)
    if (courier?.name && !entrega.entregador_nome) {
      await this.pool.query(`
        UPDATE uber_entregas SET
          entregador_nome = $1, entregador_telefone = $2, entregador_placa = $3,
          entregador_veiculo = $4, entregador_foto = $5, entregador_rating = $6,
          status_uber = 'entregador_atribuido', updated_at = NOW()
        WHERE id = $7
      `, [
        courier.name, courier.phone, courier.plate,
        courier.vehicle, courier.photo, courier.rating,
        entrega.id,
      ]);
    ...
    if (evento.location?.lat && evento.location?.lng) {
      await this.pool.query(`
        ...
        VALUES ($1, $2, $3, $4, $5)
      `, [codigoOS, evento.externalDeliveryId, evento.location.lat, evento.location.lng, evento.statusNative])
