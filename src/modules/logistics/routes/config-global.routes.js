...
function createConfigGlobalRoutes(pool, verificarToken, verificarAdmin, registrarAuditoria) {
  ...
  router.put('/config-global', verificarToken, verificarAdmin, async (req, res) => {
    try {
      ...
      const tabelaAtiva   = typeof body.tabela_preco_ativa === 'boolean' ? body.tabela_preco_ativa : undefined;
      ...
      if (tabelaAtiva !== undefined) { sets.push(`tabela_preco_ativa = $${i++}`);              vals.push(tabelaAtiva); }
