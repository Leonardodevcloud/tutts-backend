...
async function initLogisticsTables(pool) {
  ...
  // 2026-07: external_order_id UNICO por tentativa (evita "The same external_id"
  // da 99 no re-despacho). Guardado aqui pra o cancelamento usar o MESMO ref.
  await pool.query(`ALTER TABLE logistics_deliveries ADD COLUMN IF NOT EXISTS external_order_ref VARCHAR(120)`).catch(() => {});
  ...
  await pool.query(`ALTER TABLE logistics_deliveries ADD COLUMN IF NOT EXISTS alerta_10_em TIMESTAMPTZ`).catch(() => {});
  await pool.query(`ALTER TABLE logistics_deliveries ADD COLUMN IF NOT EXISTS alerta_15_em TIMESTAMPTZ`).catch(() => {});
  ...
  await pool.query(`ALTER TABLE logistics_deliveries ADD COLUMN IF NOT EXISTS rastreio_grupo_em TIMESTAMPTZ`).catch(() => {});
  ...
  // ──────────────────────────────────────────────────────────────────
  // 🆕 FASE 6 — SUNSET: renomeia as tabelas uber_* para _legacy_uber_*
  // ──────────────────────────────────────────────────────────────────
  // O Hub Logístico (logistics_*) é a única fonte. As tabelas uber_*
  // viram arquivo morto, prefixadas com _legacy_. NÃO são dropadas —
  // ficam intactas e o rename é reversível. Idempotente: ALTER TABLE
  // IF EXISTS pula se a tabela já não existir (re-deploy seguro).
  const tabelasLegado = [
    'uber_entregas', 'uber_regras_cliente', 'uber_config',
    'uber_webhooks_log', 'uber_tracking', 'uber_oauth_token',
  ];
  for (const tab of tabelasLegado) {
    try {
      await pool.query(`ALTER TABLE IF EXISTS ${tab} RENAME TO _legacy_${tab}`);
      console.log(`🗄️  [logistics] Fase 6 sunset: ${tab} → _legacy_${tab}`);
    } catch (e) {
      // se _legacy_ já existe (rename já feito), o ALTER de uma tabela
      // inexistente é no-op via IF EXISTS; outros erros só logam.
      console.log(`   [logistics] sunset ${tab}: ${e.message}`);
    }
  }
