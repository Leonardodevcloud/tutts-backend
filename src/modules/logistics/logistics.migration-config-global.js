...
async function initLogisticsConfigGlobal(pool) {
  ...
  `);
  // Tabela de precificação por distância (adicionada em fase posterior)
  await pool.query(`ALTER TABLE logistics_config_global ADD COLUMN IF NOT EXISTS tabela_preco_ativa       BOOLEAN DEFAULT false`).catch(() => {});
  ...
  await pool.query(`ALTER TABLE logistics_config_global ADD COLUMN IF NOT EXISTS preco_faixas_km JSONB`).catch(() => {}); // FAIXAS_MIGRATION_GLOBAL_V1
