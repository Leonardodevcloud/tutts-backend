...
// 🆕 2026-05 v3: Cron diário 00:10 — marca gratuidades expiradas (10 dias sem uso)
const { expirarGratuidadesVencidas } = require('./src/modules/financial/gratuidades-v3.migration');
cron.schedule('10 0 * * *', withCronLock(pool, 'expirarGratuidades', async () => {
  console.log('⏰ [CRON] Verificando gratuidades expiradas (10 dias sem uso)...');
  try {
    const expiradas = await expirarGratuidadesVencidas(pool);
    if (expiradas.length > 0) {
      console.log(`   ✅ ${expiradas.length} gratuidades marcadas como expiradas`);
    } else {
      console.log(`   ✅ Nenhuma gratuidade vencida hoje`);
    }
  } catch (err) {
    console.error('❌ [CRON] Erro ao expirar gratuidades:', err.message);
  }
...
(async () => {
  try {
    ...
    // 🆕 Fase 6 — polling do Uber legado removido (Hub Logístico assumiu).
    if (false) {
    }
    ...
    console.log('     ⏰ Gratuidades expirar — diário 00:10 🔒');
