...
// 🛡️ PID watchdog — failsafe específico pro EAGAIN. Monitora pids.current
// do cgroup; se passar de 80% do pids.max, força restart graceful.
...
async function main() {
  ...
  // 4.3. PID watchdog — failsafe específico contra "spawn EAGAIN".
  // Se PIDs do cgroup passarem de 80% do limite sustentadamente, restart
  // graceful. Isso transforma um outage de horas (até alguém perceber)
  // num glitch de poucos segundos. Defesa de ÚLTIMA instância — o fix
  // primário é o BrowserSession (browser persistente) nos agentes.
  ...
  setInterval(() => {
    ...
    // 2026-05 fix-eagain: incluir PIDs do cgroup no snapshot pra detectar
    // acúmulo de zumbis em tempo real (forma direta de validar o fix).
    let pidsInfo = '';
    try {
      const { verificarPids } = require('./src/shared/pid-watchdog');
      const fs = require('fs');
      const lerStr = (p) => { try { return fs.readFileSync(p, 'utf8').trim(); } catch (_) { return null; } };
      const curr = lerStr('/sys/fs/cgroup/pids.current') || lerStr('/sys/fs/cgroup/pids/pids.current');
      const max  = lerStr('/sys/fs/cgroup/pids.max')     || lerStr('/sys/fs/cgroup/pids/pids.max');
      if (curr) pidsInfo = `\n  PIDs cgroup: ${curr}/${max || '?'}`;
    } catch (_) {}
    logger.info(`📊 Pool snapshot:\n  ${linhas.join('\n  ')}\n  Browser pool: ${snap.browserPool.slots.filter(s => !s.livre).length}/${snap.browserPool.poolSize} ocupados${pidsInfo}`);
  }, 60_000);
